...
# -------- LOGGING -------- #
# Configuración temporal del logging (se reconfigurará después)
logging.basicConfig(
    level=logging.INFO,
...
def get_cache_directory():
    """Obtener directorio de cache que funcione en cualquier sistema"""
    # Intentar diferentes ubicaciones para el cache
    possible_cache_dirs = [
        Path.home() / ".ml_cache",  # Directorio oculto en home
        Path.home() / "ml_cache",   # Directorio visible en home
        Path.cwd() / "ml_cache",    # Directorio en la ubicación actual
        Path(tempfile.gettempdir()) / "ml_cache"  # Directorio temporal del sistema
    ]
    ...
    for cache_dir in possible_cache_dirs:
        try:
            ...
            logging.warning(f"⚠️ No se pudo usar {cache_dir}: {e}")
            continue
    
    # Si nada funciona, usar el directorio actual
    fallback_dir = Path.cwd() / "ml_cache"
    fallback_dir.mkdir(exist_ok=True)
    logging.warning(f"⚠️ Usando directorio de fallback: {fallback_dir}")
    return fallback_dir

def get_desktop_directory():
    """Obtener directorio del escritorio que funcione en cualquier sistema"""
    # Intentar diferentes ubicaciones para el escritorio
    possible_desktop_dirs = [
        Path.home() / "Desktop",  # Escritorio estándar
        Path.home() / "OneDrive" / "Desktop",  # OneDrive
        Path.home() / "OneDrive - Personal" / "Desktop",  # OneDrive personal
        Path.home() / "OneDrive - Empresa" / "Desktop",  # OneDrive empresa
        Path.home() / "OneDrive - Empresa" / "Escritorio",  # OneDrive empresa en español
        Path.home() / "OneDrive - Personal" / "Escritorio",  # OneDrive personal en español
        Path.home() / "OneDrive" / "Escritorio",  # OneDrive en español
        Path.home() / "Escritorio",  # Escritorio en español
        Path.home() / "Documents" / "Desktop",  # Alternativa
        Path.home() / "Downloads",  # Descargas como alternativa
        Path.home() / "Documents",  # Documentos como alternativa
        Path.cwd(),  # Directorio actual como fallback
    ]
    
    logging.info(f"🔍 Buscando directorio de escritorio...")
    logging.info(f"🏠 Home directory: {Path.home()}")
    logging.info(f"📂 Directorio actual: {Path.cwd()}")
    
    for desktop_dir in possible_desktop_dirs:
        try:
            logging.info(f"🔍 Probando: {desktop_dir}")
            # Intentar crear el directorio si no existe
            if not desktop_dir.exists():
                desktop_dir.mkdir(parents=True, exist_ok=True)
                logging.info(f"📁 Creado directorio: {desktop_dir}")
            
            # Verificar que podemos escribir en el directorio
            test_file = desktop_dir / "test_write.tmp"
            test_file.write_text("test")
            test_file.unlink()
            logging.info(f"✅ Desktop directory encontrado: {desktop_dir}")
            return desktop_dir
        except Exception as e:
            logging.warning(f"⚠️ No se pudo usar {desktop_dir}: {e}")
            continue
    
    # Si nada funciona, usar el directorio actual
    fallback_dir = Path.cwd()
    logging.warning(f"⚠️ Usando directorio de fallback para escritorio: {fallback_dir}")
    return fallback_dir
...
class MLAutoDownloader:
    def __init__(self):
        # Crear directorio temporal para cache (más robusto)
    ...
    def download_current_page(self):
        ...
        try:
            ...
            # Método 1: Intentar con Ctrl+S y especificar la ruta completa del cache
            pyautogui.hotkey('ctrl', 's')
            time.sleep(3)  # Más tiempo para que aparezca el diálogo
            
            # Escribir la ruta completa del cache + nombre del archivo
            cache_path = str(self.cache_dir).replace('/', '\\')  # Convertir a formato Windows
            ...
            # Verificar si se guardó en el cache (con múltiples intentos)
            for attempt in range(7):  # Intentar 7 veces
                if cache_file.exists():
                    logging.info(f"💾 Página guardada exitosamente en cache: {filename}")
                    self.last_downloaded_file = cache_file
                    return cache_file
                logging.info(f"🔄 Esperando descarga en cache... intento {attempt + 1}/7")
                time.sleep(2)  # Esperar 2 segundos más entre intentos
            
            # Si no se guardó en cache, buscar en el cache con cualquier nombre
            cache_html_files = list(self.cache_dir.glob("*.html"))
            if cache_html_files:
                cache_html_files.sort(key=lambda f: f.stat().st_mtime, reverse=True)
                latest_cache_file = cache_html_files[0]
                file_time = datetime.fromtimestamp(latest_cache_file.stat().st_mtime)
                if (datetime.now() - file_time).seconds < 60:
                    logging.info(f"💾 Archivo reciente encontrado en cache: {latest_cache_file.name}")
                    # Renombrar al nombre esperado
                    latest_cache_file.rename(cache_file)
                    self.last_downloaded_file = cache_file
                    return cache_file
            ...
            # Método 3: Buscar en múltiples ubicaciones posibles
            possible_locations = [
                Path.home() / "Downloads",
                Path.home() / "OneDrive" / "Downloads",
                Path.home() / "OneDrive - Personal" / "Downloads",
                Path.cwd(),  # Directorio actual
            ]
            
            for location in possible_locations:
                if location.exists():
                    location_html_files = list(location.glob("*.html"))
                    if location_html_files:
                        location_html_files.sort(key=lambda f: f.stat().st_mtime, reverse=True)
                        latest_file = location_html_files[0]
                        
                        file_time = datetime.fromtimestamp(latest_file.stat().st_mtime)
                        if (datetime.now() - file_time).seconds < 60:
                            logging.info(f"💾 Archivo reciente encontrado en {location.name}: {latest_file.name}")
                            new_path = cache_file
                            latest_file.rename(new_path)
                            self.last_downloaded_file = new_path
                            return new_path
    ...
    def _clean_cache(self):
        ...
        try:
            ...
            for file in self.cache_dir.glob("*.json"):
                file.unlink()
            # Limpiar reportes de debug
            for file in self.cache_dir.glob("*_debug_report_temporal.txt"):
                file.unlink()
            # NUEVO: Limpiar carpetas de archivos auxiliares (_files)
            for folder in self.cache_dir.glob("*_files"):
                if folder.is_dir():
                    import shutil
                    shutil.rmtree(folder)
            logging.info("🧹 Cache limpiado (HTML, JSON, reportes y carpetas auxiliares)")
        except Exception as e:
    ...
    def cleanup_after_processing(self):
        ...
        try:
            ...
            # Limpiar archivos JSON y reportes del cache (mantener PDF en escritorio)
            for file in self.cache_dir.glob("*.json"):
                file.unlink()
                logging.info(f"🗑️ JSON eliminado del cache: {file.name}")
            for file in self.cache_dir.glob("*_debug_report_temporal.txt"):
                file.unlink()
                logging.info(f"🗑️ Reporte eliminado del cache: {file.name}")
            ...
            for folder in self.cache_dir.glob("*_files"):
                if folder.is_dir():
                    import shutil
                    shutil.rmtree(folder)
                    logging.info(f"🗑️ Carpeta de archivos auxiliares eliminada: {folder.name}")
            
            logging.info("🧹 Cache limpiado después del procesamiento (JSON, reportes y carpetas auxiliares)")
        except Exception as e:
            logging.error(f"Error limpiando cache: {e}")
...
class MLExtractorGUI:
    ...
    def _build_ui(self):
        ...
        instr = (
            ...
            "5. El programa se cerrará al terminar\n\n"
            "OPCIÓN 2 - MANUAL:\n"
            "1. Ir Ventas/Pedidos\n"
            "2. Guardar la página completa como HTML (Ctrl+S)\n"
            "3. Seleccionar el archivo aquí"
        )
        ...
        lf.grid(row=3, column=0, columnspan=2, sticky='ew', pady=(0, 20))
        ...
        auto_frame.grid(row=4, column=0, columnspan=2, sticky='ew', pady=(0, 15))
        ...
        self.progress.grid_remove()  # Ocultar por defecto
    ...
    def _worker(self):
        try:
            ...
            # Configurar filtros por defecto (todos activos)
            ...
            # Usar el directorio de cache para reportes y JSON
            cache_dir = get_cache_directory()
            cache_dir.mkdir(exist_ok=True)
            
            # Generar reporte de debug en cache
            debug_path = cache_dir / f"{base}_debug_report_temporal.txt"
            self.status_text.set("Generando reporte detallado de extracción (en cache)...")
            ...
            json_path = cache_dir / f"{base}_productos_organizados.json"
            # El PDF se mantiene en el escritorio con nombre fijo
            outdir = get_desktop_directory()
            pdf_path = outdir / "Lista de Pedidos.pdf"
            logging.info(f"📄 PDF se guardará en: {pdf_path}")
            logging.info(f"📁 Directorio de salida: {outdir}")
            logging.info(f"📁 Directorio existe: {outdir.exists()}")
            logging.info(f"📁 Directorio es escribible: {os.access(outdir, os.W_OK) if outdir.exists() else 'N/A'}")

            self.status_text.set(f"Generando JSON organizado por secciones (en cache)...")
            extractor.save_json(json_path)

            self.status_text.set("Generando Lista de Pedidos con secciones: Urgentes | Normales | A Revisar...")
            pdfg = PDFGenerator(organized_products)
            pdfg.generate(pdf_path)
            logging.info(f"✅ PDF generado exitosamente: {pdf_path}")
            logging.info(f"📄 PDF existe: {pdf_path.exists()}")
            if pdf_path.exists():
                logging.info(f"📄 Tamaño del PDF: {pdf_path.stat().st_size} bytes")
                logging.info(f"📄 Fecha de creación: {datetime.fromtimestamp(pdf_path.stat().st_ctime)}")
            else:
                logging.error(f"❌ PDF NO existe después de generarlo: {pdf_path}")
                # Intentar crear un archivo de prueba en el mismo directorio
                test_file = outdir / "test_pdf_write.txt"
                try:
                    test_file.write_text("test")
                    test_file.unlink()
                    logging.info(f"✅ Directorio es escribible, pero PDF no se creó")
                except Exception as e:
                    logging.error(f"❌ Directorio NO es escribible: {e}")
            
            # Limpiar cache inmediatamente después de generar el PDF
            if hasattr(self, 'auto_downloader'):
                self.status_text.set("🧹 Limpiando cache...")
                self.auto_downloader.cleanup_after_processing()
    ...
    def _done(self, organized_products, filtered_count, json_path, pdf_path, debug_path):
        ...
        msg = (f"¡Proceso completado con lógica temporal de fines de semana!\n\n"
               ...
               f"📋 JSON organizado: {json_path.name} (en cache)\n"
               f"📄 Lista de Pedidos: {pdf_path.name} (en Escritorio)\n"
               f"🔍 Reporte debug: {debug_path.name} (en cache)\n"
               f"🧹 Cache limpiado automáticamente\n"
               f"Ubicación: Lista de Pedidos en Escritorio\n\n¿Abrir la Lista de Pedidos?")
...
def cli_flow(args):
    ...
    outdir = Path(args.outdir) if args.outdir else get_desktop_directory()
    outdir.mkdir(parents=True, exist_ok=True)
    
    # Usar cache para JSON y reportes
    cache_dir = get_cache_directory()
    cache_dir.mkdir(exist_ok=True)
    ...
    json_path = cache_dir / f"{base}{suffix}.json"
    pdf_path = outdir / "Lista de Pedidos.pdf"
    ...
    # Limpiar cache después de generar el PDF
    cache_dir = get_cache_directory()
    if cache_dir.exists():
        for file in cache_dir.glob("*.json"):
            file.unlink()
            logger.info(f"🗑️ JSON eliminado del cache: {file.name}")
        for file in cache_dir.glob("*_debug_report_temporal.txt"):
            file.unlink()
            logger.info(f"🗑️ Reporte eliminado del cache: {file.name}")
        # NUEVO: Limpiar carpetas de archivos auxiliares (_files)
        for folder in cache_dir.glob("*_files"):
            if folder.is_dir():
                import shutil
                shutil.rmtree(folder)
                logger.info(f"🗑️ Carpeta de archivos auxiliares eliminada: {folder.name}")
        logger.info("🧹 Cache limpiado automáticamente (incluyendo carpetas auxiliares)")
    ...
    logger.info(f"📋 JSON generado en cache: {json_path}")
    logger.info(f"📋 Reporte detallado generado en cache: {debug_path}")
    logger.info(f"📄 Lista de Pedidos generada en: {pdf_path}")
...
def main():
    # Configurar logging en el directorio de cache
    try:
        cache_dir = get_cache_directory()
        log_file = cache_dir / "ml_extractor.log"
        # Reconfigurar logging para usar el archivo en cache
        ...
        logger.warning(f"⚠️ No se pudo configurar log en cache: {e}")
